Vue.createApp({
  ...
  computed: {
    firstBook() {
      const validKeys = ["title", "isbn", "author", "publisher", "price", "numPages"]

      const filteredKeys = Object.keys(this.books[0]).filter((key) => {
  ...
  methods: {
    discount(price) {
      const justNumbers = price.split("$").join("");
      const clacDiscount = justNumbers * 0.8;
      return "$" + clacDiscount.toFixed(2);
    }
  }
}).mount("#app");
